...
async function buildSignedPDF({ name, mobile }) {
  ...
  // ── Watermark: Cityflo logo, diagonal, very light, every page ────────
  const wmScale = logo.scale(1.1);
  const wmW = wmScale.width;
  const wmH = wmScale.height;
  const angle = 35; // diagonal tilt
  const rad = angle * Math.PI / 180;
  for (const pg of pdf.getPages()) {
    const { width: pw, height: ph } = pg.getSize();
    // To rotate around the image center, offset x/y so the rotated image is centered on the page
    const cx = pw / 2;
    const cy = ph / 2;
